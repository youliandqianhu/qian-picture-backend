...
import com.qianhu.qianpicturebackend.api.aliyunai.model.GetOutPaintingTaskResponse;
...
import com.qianhu.qianpicturebackend.manager.auth.StpKit;
...
//    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    ...
    public BaseResponse<Boolean> updatePicture(@RequestBody PictureUpdateRequest pictureUpdateRequest, HttpServletRequest request) {
        ...
        pictureService.fillReviewParams(picture, currentUser);
        // 操作数据库
    ...
    public BaseResponse<Picture> getPictureById(long id, HttpServletRequest request) {
        ...
        // 空间权限校验(私人空间，就只有私人用户自己能看。管理员都看不了)
    ...
    public BaseResponse<PictureVO> getPictureVOById(long id, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(picture == null, ErrorCode.NOT_FOUND_ERROR);
        // 空间的图片，需要校验权限
        // TODO 这里使用编程式权限校验，而不是加@SaSpaceCheckPermission是因为这个接口未登录也可以调用
        // TODO 但是加了@SaSpaceCheckPermission都要求你登录过
        Space space = null;
        Long spaceId = picture.getSpaceId();
        if (spaceId != null) {
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPage(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        // 公开图库
        if (spaceId == null) {
            // 普通用户默认只能查看已过审的公开图库数据
            pictureQueryRequest.setReviewStatus(PictureReviewStatusEnum.PASS.getValue());
            // 用于处理WHERE spaceId = null 永远为假，不会返回任何行的情况
            // 这样类似于：SELECT * FROM table WHERE spaceId IS NULL
...
//            }
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        String cacheKey = "qianpicture:listPictureVOByPage:" + hashKey;

        // TODO 可是这样加缓存会不会有数据不一致的情况？是不是要在数据库更新的时候将缓存去除？
        // TODO 这样会不会出现，当我数据库数据更新了，用户还在拿着redis的缓存数据在看？没有做到数据实时同步？
        // TODO 可是我的图片上传会有审核机制,所以短时间内看不到也是正常的？要不要将缓存更新到真正用到的listPictureVOByPage方法上呢。。。
        // TODO 待实现
        ...
        // 存入 Redis 缓存
        // 10 - 15 分钟随机过期，防止雪崩
        int cacheExpireTime = 10 +  RandomUtil.randomInt(0, 5);
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCaffeine(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        String queryCondition = JSONUtil.toJsonStr(pictureQueryRequest);
        String hashKey = DigestUtils.md5DigestAsHex(queryCondition.getBytes());
        String redisKey = "listPictureVOByPage:" + hashKey;
        // 从 Caffieine 缓存中查询
