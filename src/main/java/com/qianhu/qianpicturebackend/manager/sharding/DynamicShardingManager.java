package com.qianhu.qianpicturebackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private static final String DATABASE_NAME = "logic_db"; // 配置文件中的数据库名称,但是好像只能叫logic_db。改为自己的qian_hu_picture会空指针

    // @PostConstruct 是 Java 提供的生命周期注解，它标记的方法会在 Bean 初始化完成后自动执行（构造方法之后、依赖注入完成之后）
    @PostConstruct
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "qian_hu_picture." + tableName) // 确保前缀合法
